...
Prereq: `memoryctl` installed (see [Install](/memoryctl/install/)).
...
memoryctl save --type lesson --topic skillctl-design \
  --no-confirm \
...
```sh
memoryctl read --topic skillctl-design --type decision
memoryctl read --topic skillctl-design --since 7d
memoryctl read --topic skillctl-design --format tsv     # one entry per row, agent-optimized
```
...
```sh
cd ~/work/elepay
memoryctl init                            # creates .memoryctl/topics/ + AGENTS.md block
...
```sh
cd elepay
memoryctl read --topic api-conventions --scope project
...
... memoryctl protocol rules ...
<!-- memoryctl:end -->
```

`memoryctl disable` removes only its own block, never touching skillctl's. Same for the reverse.

## What's next

- [Commands](/memoryctl/commands/) — full subcommand reference.
